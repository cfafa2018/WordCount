...
void Charcount(FILE *fp)//字符计数
{
    ...
    {
        sum++;
        if (!(str >= 0 && str <= 127) && !(temp >= 0 && temp <= 127) && flag < 3) {
            sum = sum - 1;
            flag ++;
        }
        if(flag == 2){
...
void Linecount(FILE *fp)//行计数
{
    ...
    cout<<"The num of line is:"<<sum+1<<endl;
    fclose(fp);
}

void Count(FILE *fp)//返回更复杂的数据（空行／代码行／注释行）
{
    if(fp==NULL)
    {
        cout<<"打开文件失败！"<<endl;
    ...
    while(1);
    cout<<"the num of empty-line is:"<<esum+1<<endl;
